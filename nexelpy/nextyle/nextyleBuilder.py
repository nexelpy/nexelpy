...
class Nextyle:
    ...
    def __init__(self, file: str, export_path: Optional[str] = None, layer_order: Optional[Tuple[str, ...]] = None, **custom_media_queries):
        ...
        self.scope_token = self._make_scope_token()
        target = Path(export_path) if export_path else None
        self.export_file = target if target and target.is_absolute() else (self.current_dir / target).resolve() if target else self.file_path.with_suffix(".css")
    ...
    def _make_scope_token(self) -> str:
        digest = hashlib.blake2s(str(self.file_path).encode("utf-8"), digest_size=4).digest()
    ...
    def url(self, path: str, format: Optional[str] = None) -> str:
        return self.url_resolver.url(path, format=format)
    ...
    def add_raw_css(self, raw_css: Optional[str] = None, **kwargs: str) -> "Nextyle":
        if raw_css is not None:
            resolved = self.url_resolver.resolve_raw_urls(raw_css.strip())
            self.current_context.actions.append(("raw", "base", resolved))
        for media_key, content in kwargs.items():
            resolved = self.url_resolver.resolve_raw_urls(content.strip())
